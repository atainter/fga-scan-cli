...
export function generateFgaReportHtml(report: FgaScanReport): string {
  ...
  const integrationSection =
    analysis && analysis.integrationSnippets.length > 0
      ? `<section><h2>Integration Code</h2>${analysis.integrationSnippets
          .map(
            (snippet) => `<div class="card">
  <h3>${escapeHtml(snippet.title)} <code>${escapeHtml(snippet.language)}</code></h3>
  ${snippet.appliesTo ? `<p class="meta">${escapeHtml(snippet.appliesTo)}</p>` : ''}
  <pre class="code"><code>${escapeHtml(snippet.code)}</code></pre>
</div>`,
          )
          .join('\n')}</section>`
      : '';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  pre.code { background: #1a1a2e; color: #e5e7eb; border-radius: 8px; padding: 14px 16px; overflow-x: auto; font-size: 13px; line-height: 1.5; }
...
<main>
  ...
  ${warningsSection}
  <footer>
    Generated by <code>workos scan fga</code> in ${Math.round(report.durationMs / 1000)}s${
      report.usage.total.inputTokens + report.usage.total.outputTokens > 0
        ? ` · ${(report.usage.total.inputTokens + report.usage.total.outputTokens).toLocaleString('en-US')} tokens${
            report.usage.total.costUsd > 0 ? ` · $${report.usage.total.costUsd.toFixed(report.usage.total.costUsd < 0.01 ? 4 : 2)}` : ''
          }`
        : ''
    }. This proposal is AI-generated — review it against your
